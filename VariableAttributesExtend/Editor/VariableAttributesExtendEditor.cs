...
public class VariableAttributesExtendEditor : Editor
{
	...
	private void HandleProp( SerializedProperty prop)
	{
		if( prop.name == "m_Script")
        {
			mSprictProp_ = prop.Copy();
			return;
        }
        ...
        {
			bool isSuccess = HandlePropSub(attributesArray_[type].categories, prop );
			if( !isSuccess )
            {
				attributesArray_[type].props.Add(prop.Copy());
			}
		}
        else
        {
